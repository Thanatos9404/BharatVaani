...
def get_app_state():
    if 'app_state' not in session:
        session['app_state'] = {
            ...
            'sort_by': user_prefs_cache.get('sort_by', 'date_desc') 
        }
...
def oauth2callback():
    ...
    try:
        ...
        app_state['user_picture'] = id_token.verify_oauth2_token(  
            credentials.id_token, google_requests.Request(), CLIENT_ID
        ).get('picture')
...
def dashboard():
    ...
    sort_by = request.args.get('sort_by', app_state.get('sort_by', 'date_desc'))  
    ...
    user_prefs_cache['sort_by'] = sort_by  
    save_preferences(user_prefs_cache)  
    session.modified = True
    ...
    sentiments_count = {'Positive': 0, 'Neutral': 0, 'Negative': 0, 'Unknown': 0,
                        'Error': 0}  
    top_entities_map = {}  # Initialize top_entities_map 
        ...
        for article in news_data:
            ...
            if 'published' in article and isinstance(article['published'], str):
                try:
                    ...
                    else:  # Parsing as a generic date string
                        try:
    ...
    return render_template(
        ...
        user_picture=app_state['user_picture'], 
...
def api_audio():
    ...
    lang_code = data.get('lang_code', 'en')  # 'lang_code' to match frontend
...
def generate_what_if():
    ...
    trait = request.form.get('selected_model_trait', list(WHAT_IF_MODEL_TRAITS.keys())[0])  
